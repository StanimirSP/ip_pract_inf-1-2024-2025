...
int* copy(const int* first, const int* last, int* dest)
{
...
// returns a pointer to the element past the last element copied
int* merge(const int* first1, const int* last1, const int* first2, const int* last2, int* dest, int cmp(int, int))
{
...
int main(void)
{
	...
	int merged[2 * MAX_SZ];
	int *merged_end = merge(arr1, arr1 + size1, arr2, arr2 + size2, merged, less);
